...
export async function sendOTP(_currentState: unknown, formData: FormData) {
  ...
  try {
    const response = await fetch(`${MEDUSA_BACKEND_URL}/store/auth/otp/send`, {
      method: "POST",
      headers: {
...
export async function verifyOTP(_currentState: unknown, formData: FormData) {
  ...
  try {
    ...
    // OTP verified - the backend creates/updates the customer
    // For now, we'll store a session indicator
